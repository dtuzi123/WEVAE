...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        Xtrain = Xtrain.reshape(Ntrain, -1).astype(np.float32) / 255
        Xtest = Xtest.reshape(Ntest, -1).astype(np.float32) / 255
    ...
    def Calculate_Loss(self,batch1):
        sum1 = []
        for i in range(np.shape(self.EvaluateionEachArr)[0] - 1):
            b1 = self.sess.run(self.EvaluateionEachArr[i],feed_dict={self.inputs:batch1,self.input_x:batch1})
        ...
        myCount = np.shape(self.EvaluateionEachArr)[0] - 1
        sum1 = sum1 / myCount
        sum1 = sum1 * -1
        return sum1
    ...
    def Create_VAEs(self, index, selectedComponentIndex):
        ...
        if self.componentCount == 0:
            ...
            self.EvaluateionEachArr.append(eval)
    ...
    def build_model(self):
        ...
        self.inputs_fake = tf.placeholder(tf.float32, [self.batch_size, self.data_dim])
...
#
            ...
            for t1 in range(int(np.shape(self.totalSet)[0] / self.data_stream_batch)):
                ...
                if np.shape(self.GeneratorArr)[0] == 1:
                    ...
                    for epoch in range(epochs):
                        ...
                        for idx in range(myCount):
                            batchImages = Xtrain_binarized[idx * self.batch_size:(idx + 1) * self.batch_size]
                            batchLabels = labelArr[idx * self.batch_size:(idx + 1) * self.batch_size]
                            ganBatchImages = batchImages * 2.0 - 1.0
                            ganBatchImages = np.reshape(ganBatchImages, (self.batch_size, 28, 28, 1))
                ...
                if np.shape(self.FixedMemory)[0] > self.maxMmeorySize:
                    ...
                    if np.shape(self.GeneratorArr)[0] == 1:
                        ...
                        TimeSeires.append(minScore)
                        myMin = np.mean(TimeSeires)
                        var = np.var(TimeSeires)
                        print(myMin)
                ...
                if t1 % 100 == 0:
                    batch_z = np.random.uniform(-1, 1, [self.batch_size, self.z_dim]).astype(np.float32)
                    # fake_image = generator(batch_z, training=False)
                    # fake_image = self.sess.run(fake_image)
                    # ims_cv("results/aaa" + str(t1) + ".png",merge2(fake_image[:64], [8, 8]))

                d_loss = 0
                g_loss = 0
                # d_loss = self.sess.run(d_loss)
                # g_loss = self.sess.run(g_loss)
                print("epoch {0}/{1}, step {2}/{3}, train ELBO: {4:.2f}, val ELBO: {5:.2f}, time: {6:.2f}"
                      .format(t1, 0, 0, d_loss, g_loss, self.componentCount, 0))
            ...
            myThirdName = "results/MNIST_real_57.txt"
            ...
            myThirdName = "results/MNIST_estimate_57.txt"
            f = open(myThirdName, "w", encoding="utf-8")
            for i in range(np.shape(arr1)[0]):
                f.writelines(arr1[i])
                f.writelines('\n')
