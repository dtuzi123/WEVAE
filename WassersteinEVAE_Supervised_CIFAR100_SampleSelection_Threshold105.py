...
class LifeLone_MNIST(object):
    def __init__(self):
        ...
        self.classifier_learningRate = 0.1
    ...
    def Create_VAEs(self,index,selectedComponentIndex):
        ...
        if self.componentCount == 0:
            ...
            energy = tf.reduce_sum(tf.square(reco - self.inputs), [1, 2, 3])
            self.EvaluateionEachArr.append(energy)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            createdCount = int(totalCount2 / 10.0)
            ...
            for t1 in range(int(np.shape(self.totalSet)[0] / self.data_stream_batch)):
                ...
                if t1 > createdCount and self.componentCount == 1:
                    self.Create_VAEs(self.componentCount + 1, 0)
                    self.FixedMemory = []
                ...
                if np.shape(self.FixedMemory)[0] > self.maxMmeorySize:
                    ...
                    if addCount > 300:
                        if np.shape(self.GeneratorArr)[0] == 1:
                            ...
                            timeSeriesArr.append(minScore)
                            minScore = np.mean(timeSeriesArr)
#
                ...
                print("epoch {0}/{1}, step {2}/{3}, train ELBO: {4:.2f}, val ELBO: {5:.2f}, time: {6:.2f}"
                      .format(t1, d_loss, 0, 0, 1, self.componentCount, 1))
